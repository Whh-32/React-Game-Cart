...
const Product = (props) => {
    ...
    const activChengeImage = (state,index) => {
        if (state === 'enter') {
    ...
    return (
        <div onMouseEnter={() => { activChengeImage('enter') }} onMouseLeave={() => { activChengeImage('leave') }} className='hover:scale-[103%] transition-all duration-[80ms] w-full h-[100%]  aspect-w-1 aspect-h-1 rounded-2xl overflow-hidden xl:aspect-w-6 xl:aspect-h-8 bg-[#202020]'>
            <div className='w-full xl:h-[123px] lg:h-[162px] relative'>
            ...
            <div className='w-full h-auto py-[15px] px-[10px] text-white flex flex-col'>
                ...
                <div className='text-[12px] flex justify-between border-[#696969] border-b-[1px] py-[10px]'>
                    <span className='text-[#696969]'>Release date:</span>
                    <span>{`${Rdate.month} ${Rdate.day}, ${Rdate.year}`}</span>
                </div>

                <div className='text-[12px] flex justify-between py-[10px]'>
                    <span className='text-[#696969]'>Genres:</span>
                    <span>{props.genres[0].name}</span>
                </div>

                <button className='hover:text-[#FAD860] bg-[#303030] h-[30px] mt-[10px] rounded-[5px] text-[15px] text-start pl-[10px] transition-all duration-200'>Show more details</button>
                <button className='hover:text-[#FAD860] bg-[#303030] h-[30px] mt-[10px] rounded-[5px] text-[15px] text-start pl-[10px] transition-all duration-200'>add to cart</button>
            </div>
        </div>
    )
}
